/*
 * macro to make trigger performance plots for generic reco objects.
...
void objTriggerAna(std::string configFile, std::string triggerFile, std::string inputFile, std::string outputFile)
{
    std::cout << "running objTriggerAna()" << std::endl;
    ...
    output->Close();
    std::cout << "running objTriggerAna() - END" << std::endl;
}

/*
 * run the macro without going through event loop, things done before and after the loop
 */
void objTriggerAnaNoLoop(std::string configFile, std::string inputFile, std::string outputFile)
{
    std::cout << "running objTriggerAna()" << std::endl;
    std::cout << "configFile  = " << configFile.c_str() << std::endl;
    std::cout << "inputFile   = " << inputFile.c_str()  << std::endl;
...
int main(int argc, char** argv)
{
    ...
    if (nArgStr == 5) {
        objTriggerAna(argStr.at(1), argStr.at(2), argStr.at(3), argStr.at(4));
        return 0;
    }
    else if (nArgStr == 4) {
        if (noLoop) objTriggerAnaNoLoop(argStr.at(1), argStr.at(2), argStr.at(3));
        else        objTriggerAna(argStr.at(1), argStr.at(2), argStr.at(3));
        return 0;
    }
    else if (nArgStr == 3) {
        if (noLoop) objTriggerAnaNoLoop(argStr.at(1), argStr.at(2));
        return 0;
    }
    else {
        std::cout << "Usage : \n" <<
                "./objTriggerAna.exe <configFile> <triggerFile> <inputFile> <outputFile>"
                << std::endl;
        return 1;
    }
}
...
/*
 * print information read from configurations
...
int  preLoop(TFile* input, bool makeNew)
{
    ...
    for (int iDep = 0; iDep < TRIGGERANA::kN_DEPS; ++iDep) {
        ...
        if (recoObj == RECOOBJS::kPhoton) {
            ...
            tAnaTmp.recoObj = TRIGGERANA::OBJS::kELECTRON;
        }
        tAnaTmp.dep = iDep;
