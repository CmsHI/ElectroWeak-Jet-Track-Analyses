/*
 * macro to make spectra plots for generic reco objects.
...
void objSpectraAna(std::string configFile, std::string inputFile, std::string outputFile = "objSpectraAna.root");
void objSpectraAnaNoLoop(std::string configFile, std::string inputFile, std::string outputFile = "objSpectraAna.root");

void objSpectraAna(std::string configFile, std::string inputFile, std::string outputFile)
{
    std::cout<<"running objSpectraAna()"<<std::endl;
            ...
            if (recoObj == RECOOBJS::kPhoton) {
                ...
                if (runMode[MODES::kSpectra]) {
                    for (int iAna = 0;  iAna < nSpectraAna; ++iAna) {
                        ...
                        else if (runMode[MODES::kSpectra] == MODES_SPECTRA::kMatchEle) {
                        ...
                        else if (runMode[MODES::kSpectra] == MODES_SPECTRA::kMatchProbeEle) {
    ...
    output->Close();
    std::cout<<"running objSpectraAna() - END"<<std::endl;
}

/*
 * run the macro without going through event loop, things done before and after the loop
 */
void objSpectraAnaNoLoop(std::string configFile, std::string inputFile, std::string outputFile)
{
    std::cout<<"running objSpectraAna()"<<std::endl;
    std::cout<<"configFile  = "<< configFile.c_str() <<std::endl;
    std::cout<<"inputFile   = "<< inputFile.c_str()  <<std::endl;
...
int main(int argc, char** argv)
{
    ...
    if (nArgStr == 4) {
        if (noLoop) objSpectraAnaNoLoop(argStr.at(1), argStr.at(2), argStr.at(3));
        else        objSpectraAna(argStr.at(1), argStr.at(2), argStr.at(3));
        return 0;
    }
    else if (nArgStr == 3) {
        if (noLoop) objSpectraAnaNoLoop(argStr.at(1), argStr.at(2));
        else        objSpectraAna(argStr.at(1), argStr.at(2));
        return 0;
    }
    else {
        std::cout << "Usage : \n" <<
                "./objSpectraAna.exe <configFile> <inputFile> <outputFile>"
                << std::endl;
        return 1;
    }
}
...
int readConfiguration(std::string configFile, std::string inputFile)
{
    ...
    // event cuts/weights
    // trigger requirement
    triggerPaths = ConfigurationParser::ParseList(confParser.ReadConfigValue("triggerPaths"));
