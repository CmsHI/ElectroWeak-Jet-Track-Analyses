/*
 * macro to draw branches from TTrees.
...
void drawTree(std::string configFile, std::string inputFile, std::string outputFile = "drawTree.root", std::string outputFigureName = "");
void drawTreeNoLoop(std::string configFile, std::string inputFile, std::string outputFile = "drawTree.root", std::string outputFigureName = "");

void drawTree(std::string configFile, std::string inputFile, std::string outputFile, std::string outputFigureName)
{
    std::cout<<"running drawTree()"<<std::endl;
    std::cout<<"configFile  = "<< configFile.c_str() <<std::endl;
    std::cout<<"inputFile   = "<< inputFile.c_str() <<std::endl;
    std::cout<<"outputFile  = "<< outputFile.c_str() <<std::endl;
    outputFigureStr = outputFigureName.c_str();

    if (readConfiguration(configFile, inputFile) != 0)  return;
    printConfiguration();

    std::cout<<"Input handling :"<< std::endl;
    inputSamples = InputConfigurationParser::ParseFileArgument(inputFile.c_str());
    ...
    output->Close();
    std::cout<<"running drawTree() - END"<<std::endl;
}

/*
 * run the macro without going through event loop, things done before and after the loop
 */
void drawTreeNoLoop(std::string configFile, std::string inputFile, std::string outputFile, std::string outputFigureName)
{
    std::cout<<"running drawTree()"<<std::endl;
    std::cout<<"configFile  = "<< configFile.c_str() <<std::endl;
    std::cout<<"inputFile   = "<< inputFile.c_str()  <<std::endl;
...
int main(int argc, char** argv)
{
    ...
    if (nArgStr == 5) {
        if (noLoop) drawTreeNoLoop(argStr.at(1), argStr.at(2), argStr.at(3), argStr.at(4));
        else        drawTree(argStr.at(1), argStr.at(2), argStr.at(3), argStr.at(4));
        return 0;
    }
    else if (nArgStr == 4) {
        if (noLoop) drawTreeNoLoop(argStr.at(1), argStr.at(2), argStr.at(3));
        else        drawTree(argStr.at(1), argStr.at(2), argStr.at(3));
        return 0;
    }
    else if (nArgStr == 3) {
        if (noLoop) drawTreeNoLoop(argStr.at(1), argStr.at(2));
        else        drawTree(argStr.at(1), argStr.at(2));
        return 0;
    }
    else {
        std::cout << "Usage : \n" <<
                "./drawTree.exe <configFile> <inputFile> <outputFile> (<outputFigureName>)"
                << std::endl;
        return 1;
    }
}
...
int readConfiguration(std::string configFile, std::string inputFile)
{
    ...
    formulas = ConfigurationParser::ParseList(confParser.ReadConfigValue("treeFormula"));
    selectionBase = confParser.ReadConfigValue("treeSelectionBase");
    selections = ConfigurationParser::ParseList(confParser.ReadConfigValue("treeSelection"));
    selectionSplitter = ConfigurationParser::ParseList(confParser.ReadConfigValue("treeSelectionSplitter"));
    weights = ConfigurationParser::ParseList(confParser.ReadConfigValue("TH1_weight"));

    // input for TH1
    titles = ConfigurationParser::ParseList(ConfigurationParser::ParseLatex(confParser.ReadConfigValue("TH1_title")));
    titlesX = ConfigurationParser::ParseList(ConfigurationParser::ParseLatex(confParser.ReadConfigValue("TH1_titleX")));
    titlesY = ConfigurationParser::ParseList(ConfigurationParser::ParseLatex(confParser.ReadConfigValue("TH1_titleY")));
    // nBins, xLow, xUp for TH1D histograms
    TH1D_Bins_List = ConfigurationParser::ParseListTH1D_Axis(confParser.ReadConfigValue("TH1D_Bins_List"));
    // nBinsX, xLow, xUp, nBinsY, yLow, yUp for TH2D histograms
    TH2D_Bins_List = ConfigurationParser::ParseListTH2D_Axis(confParser.ReadConfigValue("TH2D_Bins_List"));
    binsLogScaleX = confParser.ReadConfigValueFloat("binsLogScaleX");
    binsLogScaleY = confParser.ReadConfigValueFloat("binsLogScaleY");
    titleOffsetX = confParser.ReadConfigValueFloat("titleOffsetX");
    titleOffsetY = confParser.ReadConfigValueFloat("titleOffsetY");
    yMin = confParser.ReadConfigValueFloat("TH1_yMin");
    yMax = confParser.ReadConfigValueFloat("TH1_yMax");
    markerSize = confParser.ReadConfigValueFloat("markerSize");
    drawSame = confParser.ReadConfigValueInteger("drawSame");
    drawNormalized = confParser.ReadConfigValueInteger("drawNormalized");
    drawOptions = ConfigurationParser::ParseList(confParser.ReadConfigValue("drawOption"));
    markerStyles = ConfigurationParser::ParseList(confParser.ReadConfigValue("markerStyle"));
    lineStyles = ConfigurationParser::ParseList(confParser.ReadConfigValue("lineStyle"));
    fillStyles = ConfigurationParser::ParseList(confParser.ReadConfigValue("fillStyle"));
    colors = ConfigurationParser::ParseList(confParser.ReadConfigValue("color"));
    fillColors = ConfigurationParser::ParseList(confParser.ReadConfigValue("fillColor"));
    lineColors = ConfigurationParser::ParseList(confParser.ReadConfigValue("lineColor"));
    lineWidth = confParser.ReadConfigValueInteger("lineWidth");

    // input for TLegend
    legendEntryLabels = ConfigurationParser::ParseList(ConfigurationParser::ParseLatex(confParser.ReadConfigValue("legendEntryLabel")));
    legendPosition = confParser.ReadConfigValue("legendPosition");
    legendOffsetX = confParser.ReadConfigValueFloat("legendOffsetX");
    legendOffsetY = confParser.ReadConfigValueFloat("legendOffsetY");
    legendBorderSize = confParser.ReadConfigValueInteger("legendBorderSize");
    legendWidth = confParser.ReadConfigValueFloat("legendWidth");
    legendHeight = confParser.ReadConfigValueFloat("legendHeight");
    legendTextSize = confParser.ReadConfigValueFloat("legendTextSize");

    // input for text objects
    tmpText = ConfigurationParser::ParseLatex(confParser.ReadConfigValue("text"));
    ...
    tmpTextOverPad = ConfigurationParser::ParseLatex(confParser.ReadConfigValue("textAbovePad"));
    textsOverPad = ConfigurationParser::ParseList(tmpTextOverPad);
    textsOverPadAlignments = ConfigurationParser::ParseList(confParser.ReadConfigValue("textAbovePadAlign"));
    textAbovePadFont = confParser.ReadConfigValueInteger("textAbovePadFont");
    textAbovePadSize = confParser.ReadConfigValueFloat("textAbovePadSize");
    textAbovePadOffsetX = confParser.ReadConfigValueFloat("textAbovePadOffsetX");
    textAbovePadOffsetY = confParser.ReadConfigValueFloat("textAbovePadOffsetY");

    // input for TLine
    // y-axis positions of the horizontal lines to be drawn
    TLines_horizontal = ConfigurationParser::ParseListFloat(confParser.ReadConfigValue("TLine_horizontal"));
    lineStyles_horizontal = ConfigurationParser::ParseList(confParser.ReadConfigValue("lineStyle_horizontal"));
    // x-axis positions of the vertical lines to be drawn
    TLines_vertical = ConfigurationParser::ParseListFloat(confParser.ReadConfigValue("TLine_vertical"));
    lineStyles_vertical = ConfigurationParser::ParseList(confParser.ReadConfigValue("lineStyle_vertical"));
    ...
    setLogx = confParser.ReadConfigValueInteger("setLogx");
    setLogy = confParser.ReadConfigValueInteger("setLogy");
    setLogz = confParser.ReadConfigValueInteger("setLogz");
