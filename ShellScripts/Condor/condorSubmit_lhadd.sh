...
if [ $# -ne 5 ];
then
  echo "Usage: ./ShellScripts/Condor/condorSubmit_lhadd.sh <progPath> <inputFiles> <output dir> <nJobs> <nFilesPerJob>"
  echo "Example: ./ShellScripts/Condor/condorSubmit_lhadd.sh Utilities/lhadd.exe Configurations/filelists/myFiles.list /mnt/hadoop/cms/store/${USER}/outputDir/ 1 -1"
...
outputDir=$3
nJobs=$4
nFilesPerJob=$5  # number files to be processed in a single job, take care to split your file so that you run on all files. The last job might be with smaller number of files (the ones that remain).
...
Arguments    = \$(Process) $nFilesPerJob $progName $inputListName $outputDir
GetEnv       = True
...
transfer_input_files = /tmp/$proxyFile,myRun.sh,$progName,$inputListName
...
sed "\$lineStart,\$lineEnd!d" \$4 > \$inputListTmp
