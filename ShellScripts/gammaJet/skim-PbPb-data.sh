if [[ $# -ne 5 ]]
then
  echo "Usage: ./skim-PbPb-data.sh <config> <input> <output dir> <njobs> <mixing file>"
  exit 1
fi
...
cp ShellScripts/gammaJet/PbPb_Data_HiForest.list $now/
...
BASECONFS="gammaJet.conf"
BASELISTS="PbPb_Data_HiForest.list"
...
#Request_memory = 4096
Notification = Error
Executable   = $PWD/$now/skim-gammajet.sh
