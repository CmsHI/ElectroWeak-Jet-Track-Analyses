...
####################
echo "### PPSTOGRAM STEP"
echo "## zJetHistogram PP MC Madgraph ZEE"
####################
configFile=$config_PP_ZEE_MC_Madgraph

outputSkimPrefix=$skim_PP_ZEE_MC_Madgraph_Prefix
outputSkim=$outputSkimDir$outputSkimPrefix".root"
inputHist=$outputSkim

outputHistPrefix=$hist_PP_ZEE_MC_Madgraph_Prefix
...
if [ $DO_SYSVAR -eq 1 ]; then 
  ...
  configSuffixList=("${configSuffixList_hist_PP_ZEE_MC[@]}")

  cutList=("${cutList_hist_PP_ZEE_MC[@]}")

  configSuffixListSkim=("${skimSuffixList_hist_PP_ZEE_MC[@]}")
