...
int photonEnergyCorrections(const TString configFile = "CutConfigurations/gammaJet.conf",
                            const char* fn_Zee = "/mnt/hadoop/cms/store/user/tatar/official/Pythia8_Z30eeJet_Hydjet_MB/HINPbPbWinter16DR-75X_mcRun2_HeavyIon_v13_ext1-FOREST/0.root") {
    ...
    for (int i=0; i<nCentBins; ++i) {
        for (int j=0; j<nEtaBins; ++j) {
            h_pt[i][j] = new TH2F(Form("h_pt_%i_%i", i, j), Form("%i-%i%% Centrality, %.2f<|#eta|<%.2f;reco p_{T}^{#gamma};gen p_{T}^{#gamma}", centBins[0][i]/2, centBins[1][i]/2, etaBins[0][j], etaBins[1][j]), nPtBins, 0, 200, nPtBins, 0, 200);
            h_ptratio[i][j] = new TH2F(Form("h_ptratio_%i_%i", i, j), Form("%i-%i%% Centrality, %.2f<|#eta|<%.2f;reco p_{T}^{#gamma};#frac{reco p_{T}^{#gamma}}{gen p_{T}^{#gamma}}", centBins[0][i]/2, centBins[1][i]/2, etaBins[0][j], etaBins[1][j]), nPtBins, 0, 200, 50, 0, 4);
        }
    }
    ...
    TCanvas* c1 = new TCanvas("c1", "", 1500, 300 * nrows * 2 * nEtaBins);
    c1->Divide(5, nrows * 2 * nEtaBins);
    for (int i=0; i<nCentBins; ++i) {
        for (int j=0; j<nEtaBins; ++j) {
            c1->cd(i+1+(5*nrows*2)*j);
            h_ptprof[i][j] = h_ptratio[i][j]->ProfileX();
            h_ptprof[i][j]->SetTitle(Form("%i-%i%% Centrality, %.2f<|#eta|<%.2f;reco p_{T}^{#gamma};#frac{reco p_{T}^{#gamma}}{gen p_{T}^{#gamma}}", centBins[0][i]/2, centBins[1][i]/2, etaBins[0][j], etaBins[1][j]));
            h_ptprof[i][j]->SetAxisRange(0.8, 1.5, "Y");
            h_ptprof[i][j]->SetMarkerStyle(20);
            h_ptprof[i][j]->SetMarkerSize(0.4);
            h_ptprof[i][j]->SetLineColor(1);
            h_ptprof[i][j]->Draw();

            c1->cd(i+1+nrows*5+(5*nrows*2)*j);
            h_ptratio[i][j]->SetTitle(Form("%i-%i%% Centrality, %.2f<|#eta|<%.2f;reco p_{T}^{#gamma};#frac{reco p_{T}^{#gamma}}{gen p_{T}^{#gamma}}}", centBins[0][i]/2, centBins[1][i]/2, etaBins[0][j], etaBins[1][j]));
    ...
    for (int i=0; i<nCentBins; ++i)
        for (int j=0; j<nEtaBins; ++j)
            h_ecorr[i][j] = h_ptprof[i][j]->ProjectionX();

    TCanvas* c2 = new TCanvas("c2", "", 1500, 300 * nrows * nEtaBins);
    c2->Divide(5, nrows * nEtaBins);
    for (int i=0; i<nCentBins; ++i) {
        for (int j=0; j<nEtaBins; ++j) {
            c2->cd(i+1+(5*nrows)*j);
            h_ecorr[i][j]->SetTitle(Form("%i-%i%% Centrality, %.2f<|#eta|<%.2f;reco p_{T}^{#gamma};#frac{reco p_{T}^{#gamma}}{gen p_{T}^{#gamma}}", centBins[0][i]/2, centBins[1][i]/2, etaBins[0][j], etaBins[1][j]));
            h_ecorr[i][j]->SetAxisRange(0.8, 1.8, "Y");
            h_ecorr[i][j]->SetMarkerStyle(20);
            h_ecorr[i][j]->SetMarkerSize(0.4);
            h_ecorr[i][j]->SetLineColor(1);
            h_ecorr[i][j]->Draw();
        }
    }

    TFile* energyCorrectionFile = TFile::Open(configCuts.proc[CUTS::kCORRECTION].obj[CUTS::kPHOTON].s[CUTS::PHO::k_energy_correction_file].c_str(), "recreate");
    c1->Write("", TObject::kOverwrite);
    c2->Write("", TObject::kOverwrite);
    for (int i=0; i<nCentBins; ++i) {
        for (int j=0; j<nEtaBins; ++j) {
            h_ptprof[i][j]->Write(Form("h_ptprof_cent%i_eta%i", i, j), TObject::kOverwrite);
            h_ptratio[i][j]->Write(Form("h_ptratio_cent%i_eta%i", i, j), TObject::kOverwrite);
            h_ecorr[i][j]->Write(Form("photonEnergyCorr_cent%i_eta%i", i, j), TObject::kOverwrite);
        }
    }
    energyCorrectionFile->Close();

    printf("\nroot -l %s\n", configCuts.proc[CUTS::kCORRECTION].obj[CUTS::kPHOTON].s[CUTS::PHO::k_energy_correction_file].c_str());
...
int main(int argc, char* argv[]) {
    if (argc == 3)
        return photonEnergyCorrections(argv[1], argv[2]);
    else if (argc == 1)
        return photonEnergyCorrections();
    else
        printf("Usage: ./photonEnergyCorrections.exe <configFile> <Zee MC Forest>\n");
