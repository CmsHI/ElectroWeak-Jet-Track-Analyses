    ...
    for(int i=0; i<nMu_out; ++i)
    {
        for(int j=i+1; j<nMu_out; ++j)
        {
            if (doSizeCheck) {
                ...
                if(tggHiNtuplizer.muPt->size() == (unsigned)nMu)  muPt_1_out.push_back(tggHiNtuplizer.muPt->at(i));
                if(tggHiNtuplizer.muEta->size() == (unsigned)nMu)  muEta_1_out.push_back(tggHiNtuplizer.muEta->at(i));
                if(tggHiNtuplizer.muPhi->size() == (unsigned)nMu)  muPhi_1_out.push_back(tggHiNtuplizer.muPhi->at(i));
                if(tggHiNtuplizer.muCharge->size() == (unsigned)nMu)  muCharge_1_out.push_back(tggHiNtuplizer.muCharge->at(i));
                if(tggHiNtuplizer.muType->size() == (unsigned)nMu)  muType_1_out.push_back(tggHiNtuplizer.muType->at(i));
                if(tggHiNtuplizer.muIsGood->size() == (unsigned)nMu)  muIsGood_1_out.push_back(tggHiNtuplizer.muIsGood->at(i));
                if(tggHiNtuplizer.muD0->size() == (unsigned)nMu)  muD0_1_out.push_back(tggHiNtuplizer.muD0->at(i));
                if(tggHiNtuplizer.muDz->size() == (unsigned)nMu)  muDz_1_out.push_back(tggHiNtuplizer.muDz->at(i));
                if(tggHiNtuplizer.muChi2NDF->size() == (unsigned)nMu)  muChi2NDF_1_out.push_back(tggHiNtuplizer.muChi2NDF->at(i));
                if(tggHiNtuplizer.muInnerD0->size() == (unsigned)nMu)  muInnerD0_1_out.push_back(tggHiNtuplizer.muInnerD0->at(i));
                if(tggHiNtuplizer.muInnerDz->size() == (unsigned)nMu)  muInnerDz_1_out.push_back(tggHiNtuplizer.muInnerDz->at(i));
                if(tggHiNtuplizer.muTrkLayers->size() == (unsigned)nMu)  muTrkLayers_1_out.push_back(tggHiNtuplizer.muTrkLayers->at(i));
                if(tggHiNtuplizer.muPixelLayers->size() == (unsigned)nMu)  muPixelLayers_1_out.push_back(tggHiNtuplizer.muPixelLayers->at(i));
                if(tggHiNtuplizer.muPixelHits->size() == (unsigned)nMu)  muPixelHits_1_out.push_back(tggHiNtuplizer.muPixelHits->at(i));
                if(tggHiNtuplizer.muMuonHits->size() == (unsigned)nMu)  muMuonHits_1_out.push_back(tggHiNtuplizer.muMuonHits->at(i));
                if(tggHiNtuplizer.muTrkQuality->size() == (unsigned)nMu)  muTrkQuality_1_out.push_back(tggHiNtuplizer.muTrkQuality->at(i));
                if(tggHiNtuplizer.muStations->size() == (unsigned)nMu)  muStations_1_out.push_back(tggHiNtuplizer.muStations->at(i));
                if(tggHiNtuplizer.muIsoTrk->size() == (unsigned)nMu)  muIsoTrk_1_out.push_back(tggHiNtuplizer.muIsoTrk->at(i));
                if(tggHiNtuplizer.muPFChIso->size() == (unsigned)nMu)  muPFChIso_1_out.push_back(tggHiNtuplizer.muPFChIso->at(i));
                if(tggHiNtuplizer.muPFPhoIso->size() == (unsigned)nMu)  muPFPhoIso_1_out.push_back(tggHiNtuplizer.muPFPhoIso->at(i));
                if(tggHiNtuplizer.muPFNeuIso->size() == (unsigned)nMu)  muPFNeuIso_1_out.push_back(tggHiNtuplizer.muPFNeuIso->at(i));
                if(tggHiNtuplizer.muPFPUIso->size() == (unsigned)nMu)  muPFPUIso_1_out.push_back(tggHiNtuplizer.muPFPUIso->at(i));

                if(tggHiNtuplizer.muPt->size() == (unsigned)nMu)  muPt_2_out.push_back(tggHiNtuplizer.muPt->at(j));
                if(tggHiNtuplizer.muEta->size() == (unsigned)nMu)  muEta_2_out.push_back(tggHiNtuplizer.muEta->at(j));
                if(tggHiNtuplizer.muPhi->size() == (unsigned)nMu)  muPhi_2_out.push_back(tggHiNtuplizer.muPhi->at(j));
                if(tggHiNtuplizer.muCharge->size() == (unsigned)nMu)  muCharge_2_out.push_back(tggHiNtuplizer.muCharge->at(j));
                if(tggHiNtuplizer.muType->size() == (unsigned)nMu)  muType_2_out.push_back(tggHiNtuplizer.muType->at(j));
                if(tggHiNtuplizer.muIsGood->size() == (unsigned)nMu)  muIsGood_2_out.push_back(tggHiNtuplizer.muIsGood->at(j));
                if(tggHiNtuplizer.muD0->size() == (unsigned)nMu)  muD0_2_out.push_back(tggHiNtuplizer.muD0->at(j));
                if(tggHiNtuplizer.muDz->size() == (unsigned)nMu)  muDz_2_out.push_back(tggHiNtuplizer.muDz->at(j));
                if(tggHiNtuplizer.muChi2NDF->size() == (unsigned)nMu)  muChi2NDF_2_out.push_back(tggHiNtuplizer.muChi2NDF->at(j));
                if(tggHiNtuplizer.muInnerD0->size() == (unsigned)nMu)  muInnerD0_2_out.push_back(tggHiNtuplizer.muInnerD0->at(j));
                if(tggHiNtuplizer.muInnerDz->size() == (unsigned)nMu)  muInnerDz_2_out.push_back(tggHiNtuplizer.muInnerDz->at(j));
                if(tggHiNtuplizer.muTrkLayers->size() == (unsigned)nMu)  muTrkLayers_2_out.push_back(tggHiNtuplizer.muTrkLayers->at(j));
                if(tggHiNtuplizer.muPixelLayers->size() == (unsigned)nMu)  muPixelLayers_2_out.push_back(tggHiNtuplizer.muPixelLayers->at(j));
                if(tggHiNtuplizer.muPixelHits->size() == (unsigned)nMu)  muPixelHits_2_out.push_back(tggHiNtuplizer.muPixelHits->at(j));
                if(tggHiNtuplizer.muMuonHits->size() == (unsigned)nMu)  muMuonHits_2_out.push_back(tggHiNtuplizer.muMuonHits->at(j));
                if(tggHiNtuplizer.muTrkQuality->size() == (unsigned)nMu)  muTrkQuality_2_out.push_back(tggHiNtuplizer.muTrkQuality->at(j));
                if(tggHiNtuplizer.muStations->size() == (unsigned)nMu)  muStations_2_out.push_back(tggHiNtuplizer.muStations->at(j));
                if(tggHiNtuplizer.muIsoTrk->size() == (unsigned)nMu)  muIsoTrk_2_out.push_back(tggHiNtuplizer.muIsoTrk->at(j));
                if(tggHiNtuplizer.muPFChIso->size() == (unsigned)nMu)  muPFChIso_2_out.push_back(tggHiNtuplizer.muPFChIso->at(j));
                if(tggHiNtuplizer.muPFPhoIso->size() == (unsigned)nMu)  muPFPhoIso_2_out.push_back(tggHiNtuplizer.muPFPhoIso->at(j));
                if(tggHiNtuplizer.muPFNeuIso->size() == (unsigned)nMu)  muPFNeuIso_2_out.push_back(tggHiNtuplizer.muPFNeuIso->at(j));
                if(tggHiNtuplizer.muPFPUIso->size() == (unsigned)nMu)  muPFPUIso_2_out.push_back(tggHiNtuplizer.muPFPUIso->at(j));
