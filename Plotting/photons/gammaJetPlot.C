...
void setTH1TitleStyle(TH1* h);
void setTH1Style(TH1 *h, COLL::TYPE collisionType);
void setTH1_xjg(TH1* h);
void setTH1_dphi(TH1* h);
void setTH1_dphi_width(TH1* h);
void setTH1_IAA(TH1* h);

int gammaJetPlot(const TString configFile, const TString inputFile, const TString outputFile, const TString outputFigurePrefix) {
    ...
    TFile* input[nCollisionTypes] = {0};
    bool inputExists[nCollisionTypes] = {false};

    TFile* inputSys[nCollisionTypes] = {0};
    bool inputSysExists[nCollisionTypes] = {false};
    TDirectoryFile* inputSysDir[nCollisionTypes];

    std::string mcReferenceHI = "Pythia+Hydjet";
    std::string mcReferencePP = "Pythia";
    for (int i = 0; i<nCollisionTypes; ++i) {
        ...
        if (plotSystematics) {
            inputSys[i] = new TFile(inputFiles.at(i+nCollisionTypes).c_str(), "READ");
    ...
    std::string suffix_HI   = "PbPb_Data";
    std::string suffix_HIMC = "PbPb_MC";
    std::string suffix_PP   = "pp_Data";
    std::string suffix_PPMC = "pp_MC";
    std::string suffix_IAA  = "rebin";
    ...
    std::vector<bool> textWriteHiBin {
        true, true, true, true, false,
        true, false, true, false, true
    };
    std::vector<bool> textWritephoPt {
        true, true, false, false, true,
        false, true, false, true, true
    };
    std::vector<bool> textWriteDphi {
    ...
    std::vector<std::string> drawOptionsMC {
        "hist", "hist", "hist", "hist", "hist",
        "hist", "hist", "hist", "hist", "hist"
    };
    std::vector<std::string> legEntriesMC {
        "lf", "lf", "lf", "lf", "lf",
        "lf", "lf", "lf", "lf", "lf"
    };
    ...
    if (inputExists[COLL::kPP]) {
        ...
        std::vector<std::string> correlationHistNamesTMP = {"ptJet", "iaa"};
        ...
        for (int i = 0; i < sizeTMP; ++i) {
            ...
            if (iTmp > -1 && plotHI.at(iTmp))
                plotPP.at(iTmp) = false;
        }

        if (inputExists[COLL::kHIMC])
            plotPP_hist = vecFalse;
    }
    ...
    for (int i=0; i<nCorrHist; ++i) {
        for (int iPt=0; iPt<nBins_pt; ++iPt) {
            for (int iHiBin=0; iHiBin<nBins_hiBin; ++iHiBin) {
                ...
                for (int iColl = 0; iColl < nCollisionTypes; ++iColl) {
                    std::string tmpNameColl;
                    std::string tmpHistNameColl;

                    if (correlation == "iaa") {
                        tmpNameColl = Form("%s_%s", tmpName.c_str(), suffix_IAA.c_str());
                        tmpHistNameColl = Form("%s_%s", tmpHistName.c_str(), suffix_IAA.c_str());
                    } else if (iColl == COLL::kHI) {
                        tmpNameColl = Form("%s_%s", tmpName.c_str(), suffix_HI.c_str());
                        tmpHistNameColl = Form("%s_%s", tmpHistName.c_str(), suffix_HI.c_str());
                    } else if (iColl == COLL::kHIMC) {
                        tmpNameColl = Form("%s_%s", tmpName.c_str(), suffix_HIMC.c_str());
                        tmpHistNameColl = Form("%s_%s", tmpHistName.c_str(), suffix_HIMC.c_str());
                    } else if (iColl == COLL::kPP) {
                        tmpNameColl = Form("%s_%s", tmpName.c_str(), suffix_PP.c_str());
                        tmpHistNameColl = Form("%s_%s", tmpHistName.c_str(), suffix_PP.c_str());
                    } else if (iColl == COLL::kPPMC) {
                        tmpNameColl = Form("%s_%s", tmpName.c_str(), suffix_PPMC.c_str());
                        tmpHistNameColl = Form("%s_%s", tmpHistName.c_str(), suffix_PPMC.c_str());
                    }

                    if (inputExists[iColl])
                        input[iColl]->GetObject(tmpHistNameColl.c_str(), h1D[iColl][i]);
                    if (h1D[iColl][i])
                        h1DisValid[iColl][i] = true;
                ...
                if (h1DisValid[COLL::kHI][i] && plotHI.at(i)) {
                    setTH1Style(h1D[COLL::kHI][i], COLL::kHI);
                    setTH1TitleStyle(h1D[COLL::kHI][i]);
                    ...
                    if (!textWriteHiBin.at(i))
                        legEntryStr = "PbPb";
                    if (correlation.compare("iaa") != 0)    // do not put I_AA as legend entry
                        leg->AddEntry(hLegend, legEntryStr.c_str(), "pf");
                }
                if (h1DisValid[COLL::kPP][i] && plotPP.at(i)) {
                    setTH1Style(h1D[COLL::kPP][i], COLL::kPP);
                    setTH1TitleStyle(h1D[COLL::kPP][i]);
                ...
                if (h1DisValid[COLL::kHIMC][i] && plotHIMC.at(i)) {
                    ...
                    setTH1Style(h1D[COLL::kHIMC][i], COLL::kHIMC);
                    setTH1TitleStyle(h1D[COLL::kHIMC][i]);
                ...
                if (h1DisValid[COLL::kPPMC][i] && plotPPMC.at(i)) {
                    ...
                    setTH1Style(h1D[COLL::kPPMC][i], COLL::kPPMC);
                    setTH1TitleStyle(h1D[COLL::kPPMC][i]);
                ...
                if (correlation.compare("xjg") == 0) {
                    histMax = 1.6; histMin = -0.05;
                    if (iPt > 4) {
                        histMax = 2; histMin = -0.05;
                    }
                } else if (correlation.compare("dphi") == 0) {
                    histMax = 0.5; histMin = -0.05;
                    ...
                    histMax = 3; histMin = 0;
                } else if (correlation.compare("xjg_mean") == 0) {
                    histMax = 1.1; histMin = 0.6;
                } else if (correlation.compare("rjg_zNum") == 0) {
                    histMax = 1.1;
                } else if (correlation.compare("jteta") == 0) {
                    histMax = histMax+TMath::Abs(histMax)*0.4*TMath::Power(10, setLogy);
                } else if (correlation.compare("jtphi") == 0) {
                    histMax = histMax+TMath::Abs(histMax)*0.6*TMath::Power(10, setLogy);
                } else {
                    histMax = histMax+TMath::Abs(histMax)*0.6*TMath::Power(10, setLogy);
                }
                ...
                if (plotRatio.at(i)) {
                    ...
                    if (plotLowerPad_HI_PP) {
                        ...
                        setTH1Style(hTmp_lowerPad, COLL::kHI);
