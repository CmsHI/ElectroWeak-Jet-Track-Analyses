...
void zJetPlot(const TString configFile, const TString inputFile, const TString outputFile, const TString outputFigurePrefix)
{
    ...
    if (configCuts.isValid) {
        ...
        eleEta = configCuts.proc[CUTS::kPLOTTING].obj[CUTS::kELECTRON].f[CUTS::ELE::k_eleEta];

        muPt = configCuts.proc[CUTS::kPLOTTING].obj[CUTS::kMUON].f[CUTS::MUO::k_muPt];
        muEta = configCuts.proc[CUTS::kPLOTTING].obj[CUTS::kMUON].f[CUTS::MUO::k_muEta];
    ...
    std::vector<std::string> correlationHistNames   {"xjz", "xjz", "xjz_rebin", "dphi_rebin", "dphi_rebin", "dphi_rebin", "dphi_rebin", "ptJet", diLeptonM.c_str(), diLeptonPt.c_str(),
                                                     "rjz", "xjz_mean", "rjz_zNum", "xjz_mean",
                                                     "zEta", "zPhi", "jteta", "jtphi", "ptJet"};

    std::vector<std::string> versionSuffix {"final_norm", "final_norm", "final_norm", "final_norm", "final_norm", "final_norm", "final_norm", "final_norm", "final", "final_norm",
                                            "", "", "", "",
                                            "final_norm", "final_norm", "final_norm", "final_norm", "final_norm", "final_norm"};
    std::vector<std::string> jetRegion {"SIG", "SIG", "SIG", "SIG", "SIG", "SIG", "SIG", "SIG", "RAW", "RAW", "", "SIG", "", "RAW", "RAW", "RAW", "RAW", "RAW", "RAW"};
    std::vector<std::string> legendPositions {"NW", "NE", "NE", "NW", "NW", "NW", "NW", "NE", "NE", "NE", "NW", "NE", "NW", "NE", "NW", "NW", "NW", "NW", "NE"};
    std::vector<std::string> textPositions   {"NE", "NE", "NE", "NW", "NW", "NW", "NW", "NE", "NW", "NE", "SE", "NE", "SE", "NE", "NE", "NE", "NE", "NE", "NE" };
    std::vector<bool> textWriteZPt    {true, true, true, true,  true,  true,  true,  true, true, true, false, false, false, false, true, true, true, true, true};
    std::vector<bool> textWriteDphi   {true, true, true, false, false, false, false, true, true, true, true,  true,  true,  true,  true, true, true, true, true};
    std::vector<bool> textWritejetPtEtaSeparate
                                      {true, true, true, false, false, false, false, false, true, false, false,  true,  false,  false,  false, false, false, false, false};
    std::vector<std::string> drawOptionsMC   {"hist", "hist", "hist", "hist", "hist", "hist", "hist", "hist", "hist", "hist", "e", "e",   "e",   "e",   "hist", "hist", "hist", "hist", "hist"};
    std::vector<std::string> legEntriesMC    {"lf",   "lf",   "lf",   "lf",   "lf",   "lf",   "lf",   "lf",   "lf",   "lf", "lfp", "lfp", "lfp", "lfp", "lfp",  "lfp",  "lfp",  "lfp",  "lfp"};
    ...
    if (inputExists[COLL::kPP])
    {
        ...
        {"ptJet", diLeptonM.c_str(), diLeptonPt.c_str(), "zEta", "zPhi", "jteta", "jtphi", "ptJet"};
    ...
    plotSYSfromFIT[COLL::kHI] = vecTrue;
    ...
    TBox* box = new TBox();
    int boxColor = 46;
    int boxFillStyle = 3001;
    box->SetFillColorAlpha(boxColor, 0.70);
    box->SetFillStyle(1001);

    TBox* boxPP = new TBox();
    int boxColorPP = 30;
    int boxFillStylePP = 1001;
    boxPP->SetFillColorAlpha(boxColorPP, 0.70);
        ...
        for (int iPt=0; iPt < nBins_pt; ++iPt){
            for (int iHiBin=0; iHiBin < nBins_hiBin; ++iHiBin){
                ...
                for (int iColl = 0; iColl < nCollisions; ++iColl){
                    ...
                    if (plotSystematics) {
                        std::string tmpHistSysName = Form("h1D_%s_uncTot", tmpNameColl.c_str());
                ...
                if (h1DisValid[COLL::kHI][i] && plotHI.at(i)) {
                    ...
                    if (correlation.compare("xjz") == 0 || correlation.compare("xjz_rebin") == 0) setTH1_xjz(h1D[COLL::kHI][i]);
                    ...
                    if (h1DSysIsValid[COLL::kHI][i]) {  // for TLegend purposes only
                        ...
                        h1D[COLL::kHI][i]->SetFillStyle(boxFillStyle);
                        
                        h1D[COLL::kHI][i]->SetFillStyle(1001);
                    }
                ...
                if (h1DisValid[COLL::kPP][i] && plotPP.at(i)) {
                    ...
                    if (correlation.compare("xjz") == 0 || correlation.compare("xjz_rebin") == 0) setTH1_xjz(h1D[COLL::kPP][i]);
                    ...
                    if (h1DSysIsValid[COLL::kPP][i]) {  // for TLegend purposes only
                        ...
                        h1D[COLL::kPP][i]->SetFillStyle(boxFillStylePP);

                        h1D[COLL::kPP][i]->SetFillStyle(1001);
                    }
                ...
                if (h1DisValid[COLL::kHIMC][i] && plotHIMC.at(i)) {
                    ...
                    if (correlation.compare("xjz") == 0 || correlation.compare("xjz_rebin") == 0) setTH1_xjz(h1D[COLL::kHIMC][i]);
                ...
                if (h1DisValid[COLL::kPPMC][i] && plotPPMC.at(i)) {
                    ...
                    if (correlation.compare("xjz") == 0 || correlation.compare("xjz_rebin") == 0) setTH1_xjz(h1D[COLL::kPPMC][i]);
                ...
                if (correlation.compare("xjz") == 0 || correlation.compare("xjz_rebin") == 0)  {
                    histMax = 1.4;
                    histMin = -0.05;
                    // zPt > 40
                    if (iPt == 1 ) {
                ...
                else if (correlation.compare("rjz") == 0)       {histMax = 1; histMin = 0;}
                ...
                if (textWriteZPt.at(i))  textLines.push_back(Form("p_{T}^{Z} > %d GeV/c", zPt));
                ...
                if (!(correlation.compare("rjz") == 0 || correlation.compare("rjz_zNum") == 0 || correlation.compare("xjz_mean") == 0)) {
                    if (useLeptonSymbol)
                    {
...
//                float cmsTextOffset    = 0.1;  // only used in outOfFrame version
                ...
                if (legendPosition.compare("NW") == 0)  {   // leave space for latexCMS
                ...
                if (correlation.compare("xjz") == 0 || correlation.compare("xjz_rebin") == 0 || correlation.compare("dphi_rebin") == 0) {
