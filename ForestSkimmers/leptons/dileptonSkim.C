/*
 * macro to analyze and save dilepton spectrum
 */
...
void dileptonSkim(const TString configFile, const TString inputFile, const TString outputFile = "dileptonSkim.root");

void dileptonSkim(const TString configFile, const TString inputFile, const TString outputFile)
{
       std::cout<<"running dileptonSkim()"   <<std::endl;
       ...
       bool doDiElectron = (cut_nEle > 0);
       bool doDiMuon = (cut_nMu > 0);

       if (doDiElectron && doDiMuon) {
       ...
       std::cout<<"dileptonSkim() - END"   <<std::endl;
}
...
int main(int argc, char** argv)
{
    if (argc == 4)
    {
        dileptonSkim(argv[1], argv[2], argv[3]);
        return 0;
    }
    else if (argc == 3)
    {
        dileptonSkim(argv[1], argv[2]);
        return 0;
    }
    else
    {
        std::cout << "Usage : \n" <<
                "./dileptonSkim.exe <configFile> <inputFile> <outputFile>"
                << std::endl;
        return 1;
    }
}
