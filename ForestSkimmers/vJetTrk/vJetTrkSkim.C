...
void vJetTrkSkim(std::string configFile, std::string inputFile, std::string outputFile, std::string mixFile)
{
    ...
    std::vector<std::string> treeNamesHLTObj = getTreeNamesHLTObj4Skim(collisionType);
    if (isPP13tev) {
        treeNamesHLTObj = getTreeNamesHLTObjpp13TeV4Skim();
    }
    ...
    std::string treeDirHLTObj = "hltobject";
    output->mkdir(treeDirHLTObj.c_str());
            ...
            for (Long64_t j_entry_mix = 0; j_entry_mix < entriesMixTmp; ++j_entry_mix) {
                ...
                if (VJT::mixMethod == VJT::MIXMETHODS::k_match_hiBin) {
                    ...
                    iCent = getPFEnergyBin(tmpTotE, totEMax);
                         ...
                         VJT::mixMethod == VJT::MIXMETHODS::k_match_PF_HF_E_eta3to4) {

                    double minEta = -1;
                    double maxEta = -1;
                    double totEMax = -1;
                    if (VJT::mixMethod == VJT::MIXMETHODS::k_match_PF_HF_E_eta4to5) {
                        minEta = 4;
                        maxEta = 5;
                        totEMax = (isMC) ? VJT::PF_HF_E_eta4to5_max : 80000;
                    }
                    else {
                        minEta = 3;
                        maxEta = 4;
                        totEMax = (isMC) ? VJT::PF_HF_E_eta3to4_max : 55000;
                    }

                    float tmpTotE = -1;
                    if (isForest[i]) {
                        treeMixPFCand[i]->GetEntry(j_entry_mix);
                        float tmpTotE_h = getPFtotE(pfMix[i], 6, minEta, maxEta);
                        float tmpTotE_eg = getPFtotE(pfMix[i], 7, minEta, maxEta);
                        tmpTotE = tmpTotE_h + tmpTotE_eg;
                    }
                    else {
                        treeMixEventSkim[i]->GetEntry(j_entry_mix);
                        if (VJT::mixMethod == VJT::MIXMETHODS::k_match_PF_HF_E_eta4to5) {
                            tmpTotE = eventSkimMix[i].pf_h_HF_E_eta4to5 + eventSkimMix[i].pf_eg_HF_E_eta4to5;
                        }
                        else {
                            tmpTotE = eventSkimMix[i].pf_h_HF_E_eta3to4 + eventSkimMix[i].pf_eg_HF_E_eta3to4;
                        }
                    }
                    iCent = getPFEnergyBin(tmpTotE, totEMax);
                }
    ...
    for (int iFile = 0; iFile < nFiles; ++iFile)  {
        ...
        for (int i = 0; i < nTreesHLTObj; ++i) {
            ...
            std::string treePathHLTObj = Form("%s/%s", treeDirHLTObj.c_str(), treeNamesHLTObj[i].c_str());
            treesHLTObj[i] = (TTree*)fileTmp->Get(treePathHLTObj.c_str());
        }
        ...
        for (int i = 0; i < nTreesHLTObj; ++i) {
            if (iFile == 0)  {
                output->cd(treeDirHLTObj.c_str());
                outTreesHLTObj[i] = treesHLTObj[i]->CloneTree(0);
                output->cd();
            }
            else  {
                treesHLTObj[i]->CopyAddresses(outTreesHLTObj[i]);
            }
        }
        ...
        for (Long64_t j_entry = 0; j_entry < entriesTmp; ++j_entry)
        {
            ...
            if (mixEvents) {
                ...
                if (VJT::mixMethod == VJT::MIXMETHODS::k_match_hiBin) {
                    ...
                    //iCent = getPFEnergyBin((evtskim.pf_h_HF_totE + evtskim.pf_eg_HF_totE) - 982.0, totEMax);
                    //iCent = getPFEnergyBin((evtskim.pf_h_HF_totE + evtskim.pf_eg_HF_totE) - 0, totEMax);
                    // data
                    ...
                    //iCent = getPFEnergyBin((evtskim.pf_h_HF_totE + evtskim.pf_eg_HF_totE) - (660-53), totEMax);  // p, UE energy up
                    //iCent = getPFEnergyBin((evtskim.pf_h_HF_totE + evtskim.pf_eg_HF_totE) - (660+53), totEMax);  // m, UE energy down
                    ...
                    // data, Ncoll variation for extra N-N collisions before Z process
                    //iCent = getPFEnergyBin((evtskim.pf_h_HF_totE + evtskim.pf_eg_HF_totE) - (660*(1-0.25)), totEMax);
                    //iCent = getPFEnergyBin((evtskim.pf_h_HF_totE + evtskim.pf_eg_HF_totE) - (660*(1-0.50)), totEMax);
                    //iCent = getPFEnergyBin((evtskim.pf_h_HF_totE + evtskim.pf_eg_HF_totE) - (660*(1-0.75)), totEMax);
                    ...
                    if (VJT::mixMethod == VJT::MIXMETHODS::k_match_PF_HF_E_eta4to5) {
                        ...
                        iCent = getPFEnergyBin((evtskim.pf_h_HF_E_eta4to5 + evtskim.pf_eg_HF_E_eta4to5) - 393.0, totEMax);
                        // data
                        //iCent = getPFEnergyBin((evtskim.pf_h_HF_E_eta4to5 + evtskim.pf_eg_HF_E_eta4to5) - 343.0, totEMax);
                        ...
                        iCent = getPFEnergyBin((evtskim.pf_h_HF_E_eta3to4 + evtskim.pf_eg_HF_E_eta3to4) - 290.0, totEMax);
                        // data
                    }
                }
                else if (VJT::mixMethod == VJT::MIXMETHODS::k_match_hiNpix) {
